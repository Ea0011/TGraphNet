...
if __name__ == "__main__":
    ...
    gen = ChunkedGenerator_Seq2Seq(batch_size=128, cameras=[cam],
                               poses_2d=[seq_2d,], poses_3d=[seq_3d,], chunk_length=21, pad=30, shuffle=False, out_all=True)

    print(f"N Frames: {gen.num_frames()}, N Batches {gen.num_batches}")
    for cam, batch_3d, batch_6d, batch_2d, batch_edge in gen.next_epoch():
        for i in range(batch_3d.shape[0]):
            print(batch_2d.shape, batch_3d.shape)
            print("SEQS", batch_3d[i, 30:51, 0, 0].reshape(-1))
            # print("SEQS", batch_6d[i, :, 0, 0].reshape(-1)[0], batch_6d[i, :, 0, 0].reshape(-1)[-1])

    # unchunked_gen = UnchunkedGenerator_Seq(cameras=[cam, cam], poses_2d=[seq_2d, seq_2d], poses_3d=[seq_3d, seq_3d], rot_6d=[seq_6d, seq_6d], edge_feat=[seq_edges, seq_edges])
    # print(unchunked_gen.num_frames())
    # for cam, batch_3d, batch_2d, batch_6d, batch_edge in unchunked_gen.next_epoch():
    #     print(cam.shape, batch_2d.shape, batch_3d.shape, batch_6d.shape, batch_edge.shape)
