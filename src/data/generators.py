...
class ChunkedGenerator_Seq:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d, rot_6d, edge_feat,
        ...
        for i in range(len(poses_2d)):
            ...
            # print(f"Chunks {n_chunks} offset: {offset} bounds: {bounds}")
            if augment:
                pairs += zip(np.repeat(i, len(bounds - 1)), bounds[:-1], bounds[1:], ~augment_vector)
...
if __name__ == "__main__":
    ...
    gen = ChunkedGenerator_Seq(batch_size=1, cameras=None,
                               poses_2d=[seq_2d,], poses_3d=[seq_3d,], rot_6d=[seq_6d,],
                               edge_feat=[seq_edges,], chunk_length=243, pad=0)
    ...
    unchunked_gen = UnchunkedGenerator_Seq(cameras=None, poses_3d=[seq_3d], poses_2d=[seq_2d], rot_6d=[seq_6d], edge_feat=[seq_edges])
    print(unchunked_gen.num_frames())
    for cam, batch_3d, batch_2d, batch_6d, batch_edge in unchunked_gen.next_epoch():
        print(batch_2d.shape, batch_3d.shape, batch_6d.shape, batch_edge.shape)
        data = eval_data_prepare(243, torch.from_numpy(batch_2d), torch.from_numpy(batch_edge), torch.from_numpy(batch_3d), torch.from_numpy(batch_6d))
        print(data[0].shape)
