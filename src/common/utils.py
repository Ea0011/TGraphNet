...
def save_checkpoint_pos_ori(state, is_best, is_best_pos, is_best_ori, checkpoint):
    """Saves model and training parameters at checkpoint + 'last.pth.tar'. If is_best==True, also saves
    checkpoint + 'best.pth.tar'
    Args:
        state: (dict) contains model's state_dict, may contain other keys such as epoch, optimizer state_dict
    ...
    if is_best_pos:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'best_pos.pth.tar'))

    if is_best_ori:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'best_ori.pth.tar'))
...
def load_checkpoint(checkpoint, model, optimizer=None, scheduler=None, last_best=None):
    ...
    if last_best:
        ...
        best_val_geod_err = checkpoint['last_best_geod_err']

        return checkpoint, best_val_err, best_val_geod_err
    else:
        return checkpoint
...
def get_adj_weight(checkpoint):
    if not os.path.exists(checkpoint):
        raise("File doesn't exist {}".format(checkpoint))
    checkpoint = torch.load(checkpoint)
    state_dict = checkpoint['state_dict']
    return state_dict['adj'][0]
...
def write_train_summary_scalars(writer, experiment, epoch, batch_idx, num_batches, summary):
    ...
    for key, value in error.items():
        writer.add_scalar("error/"+str(key), value, epoch)

    for key, value in loss.items():
        writer.add_scalar("loss/"+str(key), value, epoch)


def write_val_summary_joint(writer, epoch, summary_epoch, detailed=False):
    error = summary_epoch['error']
    loss = summary_epoch['loss']
    for key, value in error.items():
        writer.add_scalar("error/"+str(key), value, epoch)
    ...
    if detailed:
        joint_metrics = summary_epoch['joint']
        for key, value in joint_metrics['val_pos'].items():
